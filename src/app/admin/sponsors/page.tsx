...
function SponsorModal({ sponsor, onClose, onSave, isNew }: { sponsor: Omit<Sponsor, "id" | "addedAt">; onClose: () => void; onSave: (d: Omit<Sponsor, "id" | "addedAt">) => void; isNew?: boolean }) {
  ...
  const sel = "w-full bg-[var(--bg-surface)] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-[#C9921A]/60";
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-10 overflow-y-auto">
      ...
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="relative w-full max-w-xl bg-[var(--bg-surface)] rounded-2xl border border-white/10 overflow-hidden mb-4">
